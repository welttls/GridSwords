...
6. **【「关机」术语约定（2026-08-13 用户定）】**：用户说「关机」= **关闭开发服务器 / 结束本次 AI 协作会话**（停掉 `npm run dev`、终止当前对话），**绝不对操作系统执行 `shutdown` 命令**。需要停服时 kill 终端/结束会话即可，不要碰物理机电源。
...
- **v2.7.1（2026-08-13）**：整体代码审查修复——①血统链 lineage 随档保存/恢复（隔代血亲/悟道树续档不裂）；②`exportSave` 补 `dailyDropKind/dailyDropLocked`；③剑域纪事 Chronicle 随档保存（`Chronicle.restore`，刷新续玩不丢事件）；④熔岩死因显式 `die('lava')`（成就「地形大师」可达）；⑤晋升弹窗补 onClose 防死循环；⑥面板/布阵关闭恢复 wasPaused、败局弹窗清渲染器/HUD、`showEmbryoSelect` 同步销毁；⑦粒子共享 Graphics 每帧重绘（500→1 draw call）、暂停无特效跳过渲染、大比日志上限 200 行；⑧存档防御（坏档备份/结构校验/写入失败 toast）；⑨剑谱祖辈/后裔陨落文案区分。build 零错误、headless/实机验证通过。
