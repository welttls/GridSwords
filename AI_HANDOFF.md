...
4. **天劫**：第 10 日食物停生、边界向内收缩(每 8 tick 一格)至 4×4，幸存者评本命剑；v1.10.0 起失败(无幸存者)弹「剑意尽灭」界面(存活峰值/全灭日 + 重新炼剑快捷入口)，仍不得剑尘。
...
**词条系统**(`AffixDB.ts`，存于 `Genome.affixes` 可遗传)：eat30 吞金(采气≥20) / kill5 斩念(击破≥3) / fight15 百炼(历经≥25战) / roam400 游历(足迹≥350 且密度≥0.35/tick，即每时辰≥28 格) / poison 淬毒(rare，锋锐≥7 且杀性≥0.55 且存续≥2500且历经≥15战) / parasite 寄灵(rare，木行+合击+世代≥4)。判定在 `SwordAgent.recheckAffixes()` 每 tick 参悟。**v1.8.0 门槛放宽、v1.9.0 淬毒加行为要求(存续2500/15战)、v1.11.0 百炼 25 战 / 游历改足迹密度(苟活久者不悟)**。
